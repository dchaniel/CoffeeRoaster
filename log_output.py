...
def setup_plot():
    ...
    lines = [
        ax[0].plot([], [], "-", label="Actual Temperature")[0],
...
def plot_live(debug=False):
    ...
    try:
        while True:
            ...
            try:
                actual_temp, desired_temp, heater_status = map(
                    float, serial_line.split(",")
                )
                ...
                if debug:
                    print(
                        f"Actual Temp: {actual_temp}, Desired Temp: {desired_temp}, Heater Status: {heater_status}"
                    )
                ...
                ax[1].set_ylim(
                    -0.1, 1.1
                )  # Ensure the heater status plot y-axis remains 0 or 1
                ax[1].set_xlabel("Time (s)")
                ax[1].set_ylabel("Heater Status")
                ...
                # Log data
                log_file.write(
                    f"{elapsed_time},{actual_temp},{desired_temp},{heater_status}\n"
                )
                log_file.flush()
            except ValueError:
                pass
