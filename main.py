...
async def control_loop():
    ...
    while True:
        ...
        shared_state["ideal_temperature"] = linear_interpolation(
            shared_state["current_time"], times, temperatures
        )

        if (
            shared_state["current_temperature"]
            < shared_state["ideal_temperature"] - deadband
        ):
...
async def log_data():
    while True:
        ...
        print(
            "{:.2f}, {:.2f}, {}".format(
                current_temperature, ideal_temperature, heater_status
            )
        )
...
asyncio.run(
    main()
)  # Uncomment this line if running in an environment that supports asyncio.run
